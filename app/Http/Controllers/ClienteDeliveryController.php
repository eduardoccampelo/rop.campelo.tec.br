...
class ClienteDeliveryController extends Controller
{
	...
	public function alterarSenha($id){
		...
		return view('clienteDelivery/alterar_senha')
		->with('title', 'Alterar senha')
		->with('cliente', $cliente);
	}

	public function alterarSenhaStore(Request $request){
		$senha = $request->senha;
		$repitaSenha = $request->repita_senha;

		if(strlen($senha) == 0){
			session()->flash('mensagem_erro', 'Digite a senha!');
			return redirect()->back();
		}

		if($senha != $repitaSenha){
			session()->flash('mensagem_erro', 'Senhas não coencidem!');
			return redirect()->back();
		}
		...
		session()->flash("mensagem_sucesso", "Senha de cliente $cliente->nome alterada!");
		return redirect('/clientesDelivery');
