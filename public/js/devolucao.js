...
$(function () {
	if($('#itens_nf').val()){
		...
		ITENSDEV = JSON.parse($('#itens_dev').val());
	}
...
function openModal(data){
	...
	$('#ncm').val(data.ncm)
	$('#cfop').val(data.cfop)
	$('#cst_csosn').val(data.cst_csosn).change()
	$('#cst_pis').val(data.cst_pis).change()
	$('#cst_cofins').val(data.cst_cofins).change()
	$('#cst_ipi').val(data.cst_ipi).change()
	$('#perc_icms').val(data.perc_icms)
	$('#perc_pis').val(data.perc_pis)
	$('#perc_cofins').val(data.perc_cofins)
	$('#perc_ipi').val(data.perc_ipi)
	$('#modal1').modal('show');
}
