...
$('#salvar').click(() => {
	...
	({
		...
		success: function(e){
			...
			$("#th_"+codigo).removeClass('text-danger');
...
function selectProd(id){
	...
	.done((res) => {
		...
		$('#valor_venda2').val(parseFloat(PRODUTO.valor_venda).toFixed(2))
		$('#valor_compra2').val(parseFloat(PRODUTO.valor_compra).toFixed(2))
...
$('#salvarLink').click(() => {
	...
	if(prod != 'null'){
		...
		$('#th_'+id).html(produto.nome)
		$('#th_'+id).removeClass('text-danger')
