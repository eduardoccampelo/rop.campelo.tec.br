...
<div class="card card-custom gutter-b">
	<div class="card-body">
		...
		<button class="btn btn-info" data-toggle="modal" data-target="#modal-pedidos">
		...
		<div class="row align-items-center">
			...
			<div class="form-group validated col-sm-5 col-lg-5 col-10"><br>
				...
				<a href="/pedidosDelivery/frente" class="btn btn-sm btn-danger">Novo cliente</a>
			</div>
			@endif
		...
		<div class="card card-custom gutter-b">
			<div class="card-body">
				...
				<form id="formItem" class="col s12" method="post" action="/pedidosDelivery/saveItemCaixa">
						...
						<div class="card card-custom gutter-b example example-compact">
							<div class="card-header">
								<div class="row" style="height: 72px; overflow-x: auto; width: auto; white-space: nowrap">
									<div class="form-group validated col-sm-12 col-lg-12 col-12 col-sm-12" style="margin-top: 10px;">
										<a href="#!" id="cat_todos" onclick="categoria('todos')" style="height: 40px; min-width: 80px;" class="label label-xl label-inline label-light-muted ativo">Mais vendidos</a>
							...
							<div class="card-body">
								<div class="row" id="prods">
									...
									<div class="col-sm-12 col-lg-6 col-md-6 col-xl-3" id="atalho_add">
										<div class="card card-custom gutter-b example example-compact">
											<div onclick="adicionarProdutoRapido('{{$p->id}}')" class="card-header" style="height: 200px;">
												@if($p->produto->imagem)
												<img class="img-prod" src="/imgs_produtos/{{$p->produto->imagem}}">
												@else
											...
											<button type="button" onclick="adicionarProdutoRapidoComAdicional('{{$p->id}}')" class="btn btn-info">
												+ Adicional
											</button>
										</div>
		...
		@if(isset($pedido))
		<form id="formPedido" method="get" action="/pedidosDelivery/frenteComPedidoFinalizar">
			<div class="card card-custom gutter-b">
				<div class="card-body">
					<div class="row align-items-center">
						...
						<div class="col-sm-2 col-lg-2 col-6">
							<input type="hidden" value="{{$pedido->id}}" name="pedido_id">
							<button style="width: 100%; margin-top: 13px;" id="btn-salvar" type="button" @if(!isset($pedido) || sizeof($pedido->itens) == 0) disabled @endif class="btn btn-lg btn-success">Salvar</button>
						</div>
					</div>
				</div>
			</div>
			...
			<form method="post" action="/pedidosDelivery/novoClienteDeliveryCaixa">
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="form-group validated col-sm-4 col-lg-4">
							<label class="col-form-label" id="">Código (opcional)</label>
							<div class="">
...
<div class="modal fade" id="modal-pedidos" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row" style="margin-top: 5px;">
					<a href="/pedidosDelivery/irParaFrenteCaixa/{{$p->id}}" class="btn btn-{{$colors[$rand]}}">
						Pedido {{$p->id}} - R$ {{number_format($p->valor_total, 2, ',', '.')}} | {{$p->cliente->nome}} {{$p->cliente->sobre_nome}} | Endereço: @if($p->endereco_id != null) {{$p->endereco->rua}}, {{$p->endereco->numero}} {{$p->endereco->bairro}} @else Balcão @endif
					</a>
				</div>
				@endforeach
			</div>
...
<div class="modal fade" id="modal-adicional" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
			...
			<div class="modal-body">
				...
				<div class="row align-items-center">
					<div class="form-group validated col-sm-4 col-lg-4 col-12">
						<label class="col-form-label" id="">Adicionais</label><br>
						<select class="form-control select2" @if(!isset($pedido)) disabled @endif style="width: 100%" id="kt_select2_5">
							@foreach($adicionais as $a)
							<option value="{{$a->id}}">{{$a->id}} - {{$a->nome}} - R${{$a->valor}}</option>
							@endforeach
						</select>
					</div>
					<div class="col-sm-1 col-lg-1 col-3" id="btn-add-adicional2">
						<a style="margin-top: 12px;" class="btn btn-light-info">
							<i class="la la-plus"></i>
						</a>
					</div>
					...
					<div class="form-group col-lg-4 col-md-4 col-sm-6 col-6">
						...
						<div class="">
							<div class="input-group">
								<input type="text" name="observacao" class="form-control" id="observacao-item"/>
							</div>
						</div>
					</div>
					<div class="form-group col-lg-2 col-md-2 col-sm-6 col-6">
						...
						<div class="">
							<div class="input-group">
								<input value="1" type="text" name="qtd-tem" class="form-control money" id="qtd-item"/>
							</div>
						</div>
					</div>
				</div>
				<div id="adicioanais-html2" style="display: none;">
					<div class="row">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-light-danger font-weight-bold" data-dismiss="modal">Fechar</button>
				<button type="button" id="saveItemAdicional" class="btn btn-light-success font-weight-bold" data-dismiss="modal">Salvar</button>
			</div>
		</div>
	</div>
</div>
